...
% - makes rho be random from a beta distribution
...
nRep = 1000; IsampE = cell(1, lenE); IsampTemp = repmat(Iloc, [nRep 1]);
% Fractions of and total cases from false additions over replicates
fsampE = IsampE; fsampTemp = IsampTemp; ItotE = IsampE;

% Parameters of various beta distributions
fr = rho(1:end-1)./(1 - rho(1:end-1)); lenr1 = length(fr);
b = 200*ones(1, lenr1); a = fr.*b; 

y = cell(1, lenr1);
x = 0:0.01:1; my = zeros(1, lenr1);

% Sampling distributions with mean my = rho
figure;
hold on;
for i = 1:lenr1
    y = betapdf(x, a(i), b(i));
    my(i) = betastat(a(i), b(i));
...
if saveTrue
    cd(saveFol);
    save(['sampBeta_' num2str(nday) '_' num2str(lenE) '_' num2str(nRep) '.mat']);
    cd(thisDir);
end
...
for i = 1:lenE
    ...
    for j = 1:tst
        % Add extra variation via beta
        if i == lenE
            % At rho = 1 no variation
            IsampTemp(:, j) = binornd(Iloc(j), rho(i), [nRep 1]);
        else
            % Beta distributed variation
            IsampTemp(:, j) = binornd(Iloc(j), betarnd(a(i), b(i)), [nRep 1]);
        end
    end

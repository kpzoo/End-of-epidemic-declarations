...
%priors.a = 5; priors.b = 10^-2; % for EVD and boom-bust
priors.a = 1; priors.b = 5; % for EVD and control
...
scenExplore = [4]; nScen = length(scenExplore); 
tsim = zeros(1, nScen);
...
for jj = 1:nScen
    ...
    while(ii <= M)
        ...
        [tdecs(ii, :), tdec0(ii), didEnd(ii), z0, z, ~, ~, ~, ~, ~] =...
            getSingleEpidFnDiseaseNoQ(Iday{ii}, Lam{ii}, nday(ii), priors, distvals, Rtrue{ii}, k, mu);
        
        % Check on a bad case
        if abs(tdecs(ii, 1) - tdec0(ii)) > 8
            assignin('base', 'zerr', z); assignin('base', 'z0err', z0);
            assignin('base', 'Ierr', Iday{ii});
            warning('Big difference in estimates');
        end

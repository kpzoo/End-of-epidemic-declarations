...
% Folder for saving
saveFol = 'resurgence data'; thisDir = cd;
...
scenNam = {'control', 'recovery', 'cascade', 'boom-bust', 'waves'};
...
disp(['True R scenario: ' distNam{distNo}]);
...
% Alter Rs for resurgence
Rs = [2 0.1 2.2]; ts = [150 200];

% Starting times (days)
tday0 = 1:301; nday0 = length(tday0);
tcheck = max(ts);
...
k = 30; disp(['Window length: ' num2str(k+1)]);
% set window to 100 for general
...
if didEnd
    ...
    % WHO end time (42 days)
    ilast = find(Iday, 1 , 'last'); tlast = tindex(ilast-1);
    twho = tlast + 42; % should really be from recovery time
    % Approx end time from z
    ee = abs(z - 1); idee = find(ee < 10^-5, 1, 'first');
    tee = tindex(idee);
else
    idmax = length(tindex);
end
...
if wholeEp 
    ...
    ylabel('$z_s$', 'FontSize', 18);
    %xlim([tindex(1) max(tdecs)+30]);
    h = gca; h.YColor = h.XColor;
    h.YLim = [0 1.05];
    legend('$z_s^*$', '$z_s$', 'Location', 'Best');
    yyaxis left

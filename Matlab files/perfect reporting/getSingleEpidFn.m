...
function [tdecs, tdec0, didEnd, z0, z, z1, win, R, tz, idtz, tdec0q, tdecq, tdec1q] = ...
    getSingleEpidFn(Iday, Lam, nday, priors, distvals, Rtrue, k, qs, mu)
...
for i = 1:lenind
    ...
    [z0(i), ~] = getProbEliminTrue(Itemp{i}, Rtrue(tindex(i)), distvals);
end

% Elimination probabilities with estimated R
[~, R, ~, z, z1, tdecs, win] = eliminProbFn(k, Iday, nday, Lam, priors, distvals, lenind, Itemp, mu);

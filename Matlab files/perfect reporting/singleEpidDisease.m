...
% Choose a scenario and disease serial interval
scenNo = 2; distNo = 1;
...
disp(['Disease: ' distNam{distNo}]);

% Set serial interval parameters
[distvals.pm, Rs, ts, distvals.omega] = setupSIandRGamma(distNo, scenNo);
...
while Iwarn 
    [Iday, Lam, Rtrue, tday, Iwarn] = epiSimDieDisease(length(tday0), ts, Rs, scenNo, distvals);
end
...
for i = 1:lenind
    ...
    [z(i), zseq{i}, z1(i), zseq1{i}, Rseq{i}] = getProbEliminDisease(k, Itemp, priors, distvals);
    % True elimination probabilities given R
    [z0(i), zseq0{i}] = getProbEliminTrueDisease(Itemp, Rtrue(tindex(i)), distvals);
end
...
disp(['Difference in tdec and tdec0: ' num2str(tdecs(2) - tdecs(1))]);
...
if didEnd
    ...
    if wholeEp && strcmp(distChoice, 'EVD')
        % WHO end time (42 days fir EVD)
        ilast = find(Iday, 1 , 'last'); tlast = tindex(ilast-1);
        twho = tlast + 42; 
        % Add 5 days for recovery time (see Van Kerkhove 2015)
        twho = twho + 5;
        % Approx end time from z
        ee = abs(z - 1); idee = find(ee < 10^-2, 1, 'first');
        ee0 = abs(z0 - 1); idee0 = find(ee0 < 10^-2, 1, 'first');
        tee = tindex(idee); tee0 = tindex(idee0);
    end
else
    idmax = length(tindex);
end
...
if wholeEp && didEnd
    ...
    figure;
    yyaxis left
    stairs(tindex(1:idmax), Iday(tindex(1:idmax)), 'Color', [0.3 0.75 0.93], 'LineWidth', 2);
    grid off; box off;
    ylabel('$I_s$');
    h = gca; h.YColor = h.XColor;
    xlim([tindex(1) max(tdecs)+30]);
    yyaxis right
    stairs(tindex, z0, '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 2);
    hold on;
    stairs(tindex, z, '-', 'Color', [0.85 0.33 0.1], 'LineWidth', 2);
    plot(1:length(z), mu*ones(size(z)), 'k--', 'LineWidth', 2);
    % For EVD have twho
    if strcmp(distChoice, 'EVD')
        stairs([twho twho], [0 1], 'm--', 'LineWidth', 2);
    end
    grid off; box off; hold off;
    xlabel('$s$ (days)');
    ylabel('$z_s$');
    xlim([tindex(1) max(tdecs)+50]);
    h = gca; h.YColor = h.XColor;
    h.YLim = [0 1.05];
    legend('$I_s$', '$z_s^*$', '$z_s$', 'Location', 'Best', 'FontSize', 18, 'Box', 'off');
    
    if saveTrue
        cd(saveFol);
        saveas(gcf, ['exwhoEVD_' scenChoice '_' num2str(k)], 'fig');
        cd(thisDir);
    end 
end
...
if saveTrue
    cd(saveFol);
    save([scenChoice 'EVD_' num2str(nday) '_' num2str(k) '.mat']);
    cd(thisDir);
end